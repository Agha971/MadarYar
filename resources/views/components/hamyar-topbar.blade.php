<header class="border-b border-slate-200 bg-white/90 backdrop-blur dark:border-slate-800 dark:bg-slate-900/90">
    <div class="flex items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <div>
            <h1 class="text-lg font-extrabold text-slate-900 dark:text-white">{{ $pageTitle ?? 'پنل همیاران' }}</h1>
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ $pageDescription ?? 'مدیریت همکاری و پیگیری وضعیت حساب' }}</p>
        </div>
